...
#         self.current_send_task = None


# broadcast_queue = broadcast_queue = asyncio.Queue(maxsize=10)
...
#             # Cancel any pending send for this client
#             if client.current_send_task and not client.current_send_task.done():
#                 client.current_send_task.cancel()
                
#             # Create new send task
#             send_task = asyncio.create_task(send_with_timeout(client.websocket, message))
#             client.current_send_task = send_task
#             tasks.append(send_task)
        
#         # Wait for all sends with timeout
#         done, pending = await asyncio.wait(tasks, timeout=1.0)
        
#         # Handle disconnected clients only
#         disconnected = set()
#         for client in connected_clients.copy():
#             try:
#                 if client.current_send_task.done():
#                     await client.current_send_task  # Will raise if client disconnected
#             except websockets.exceptions.ConnectionClosed:
#                 disconnected.add(client)
                
#         connected_clients.difference_update(disconnected)

#     async def send_with_timeout(websocket, message):
#         try:
#             await asyncio.wait_for(websocket.send(message), timeout=0.05)
#         except asyncio.TimeoutError:
#             # Just let timeout happen, client stays connected
#             print("Timeout sending message to client")
#             pass

#     async def update_broadcaster():
#         while True:
#             try:
#                 # Use get_nowait() to check queue non-blocking
#                 try:
#                     broadcast_data = broadcast_queue.get_nowait()
#                 except asyncio.QueueEmpty:
#                     await asyncio.sleep(0.01)  # Sleep when queue is empty
#                     continue
                    
#                 # Rate limit broadcasts
#                 await asyncio.sleep(0.01)  # Max 100 broadcasts per second
                
#                 try:
#                     message = json.dumps(get_value(broadcast_data))
#                 except Exception as e:
#                     print(f"Error serializing broadcast data: {e}")
...
#             except Exception as e:
#                 print(f"Exception in update_broadcaster: {e}")
#                 await asyncio.sleep(0.1)

#     def append_to_broadcast_queue(broadcast_data):
#         try:
#             if broadcast_queue.full():
#                 print("Warning: broadcast queue full, dropping message")
#                 return
                
            
#             future = asyncio.run_coroutine_threadsafe(
#                 broadcast_queue.put(broadcast_data),
...
#             broadcast_msg_header = broadcast_data['update_type']
#             print(f"Broadcast queued: {broadcast_msg_header}")
#         except Exception as e:
            
#             print(f"Error queueing broadcast: {e}")
...
#                     if data['command'] in ['add_points', 'update_points', 'remove_points']:
...
if __name__ == '__main__':
    ...
    async def check_for_updates():
        await work_on_affine()
        await work_on_points()
