...
async def connect_to_source():
    async with websockets.connect("ws://10.0.63.153:4002/ws") as source_ws:
...
async def main():
    ...
    async with websockets.serve(relay_server, "0.0.0.0", 4004):
        print("Relay server started on ws://0.0.0.0:4004 (all interfaces)")
